      ...
      {items.map((number) => {
        return (
          <div
            ...
            key={number}
            className="overflow-hidden relative transition duration-700 ease-in-out hover:scale-95 rounded "
          >
            {pics[number].star && <Star />}
            <div
              className={
            ...
            >
              <div className="self-center opacity-100 absolute text-center text-xs md:text-xl text-[#203e64] p-2 ">
                <div className="border-4 border-[#203e64] bg-white p-2 ">{pics[number].text}</div>
              </div>
            </div>
          </div>
        );
